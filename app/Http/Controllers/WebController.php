...
class WebController extends Controller
{
    ...
    function DoctorName($code, $array)
    {
    ...
    function ClinicName($code, $array)
    {
    ...
    function ProcedureName($code, $array)
    {
    ...
    function setTime($prescript, $closeVisit)
    {
        if($closeVisit == '9'){
    ...
    function setAssessmentText($value, $type)
    {
        switch ($type) {
            case 'isolation':
                switch ($value) {
    ...
    function setCloseVisit($visitcode)
    {
    ...
    function result(Request $req)
    {
        ...
        if($master == null){
            ...
            $Clinics = DB::connection('SSB')
                ->table('DNSYSCONFIG')
                ->where('CtrlCode ', 42203)
                ->select('CtrlCode','Code','LocalName')
            ->get();
            $Doctors = DB::connection('SSB')
                ->table('HNDOCTOR_MASTER')
                ->select('Doctor','LocalName',)
            ->get();
            $Procedures = DB::connection('SSB')
                ->table('DNSYSCONFIG')
            ...
            $Prescripts = DB::connection('SSB')
                ->table('HNOPD_PRESCRIP')
            ...
            foreach($Labs as $indexLab => $lab){
                ...
                $labToday = false;
                switch ($lab) {
                    case  $lab->EntryDateTime == $date && $lab->AppointmentNo == null && $lab->Ward == null:
                        $labToday = true;
                    break;
                    case  $lab->EntryDateTime == $date && $lab->EntryDateTime == $lab->SpecimenReceiveDate:
                        $labToday = true;
                    break;
                }
                if($showDebug){
            ...
            $Xrays = DB::connection('SSB')
                ->table('HNXRAYREQ_HEADER')
            ...
            foreach ($Xrays as $indexXray => $xray) {
                ...
                if($xray->AcknowledgeDateTime !== null){
                    ...
                    $xray->AcknowledgeDateTime = date('Y-m-d', strtotime($xray->AcknowledgeDateTime));
                }
                ...
                switch ($xray) {
                    case $xray->EntryDateTime == $date && $xray->AppointmentNo == null && $xray->AcknowledgeFlag == 1:
                        $xrayToday = true;
                    break;
                    case $xray->EntryDateTime == $date && $xray->EntryDateTime == $xray->AcknowledgeDateTime && $xray->AcknowledgeFlag == 1:
                        $xrayToday = true;
                    break;
